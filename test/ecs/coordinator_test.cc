...
class CoordinatorTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    test_coordinator = std::make_unique<tbge::ecs::Coordinator>();
  ...
  std::unique_ptr<tbge::ecs::Coordinator> test_coordinator;
};
...
TEST_F(CoordinatorTest, CreateEntity) {
  tbge::ecs::Entity entity1 = test_coordinator->CreateEntity();
  tbge::ecs::Entity entity2 = test_coordinator->CreateEntity();
  tbge::ecs::Entity entity3 = test_coordinator->CreateEntity();
...
TEST_F(CoordinatorTest, RegisterComponentType) {
  tbge::ecs::ComponentManager* component_manager =
...
TEST_F(CoordinatorTest, AddComponent) {
  ...
  test_coordinator->RegisterComponentType<DummyComponent>();
  tbge::ecs::Entity entity = test_coordinator->CreateEntity();

  tbge::ecs::Signature expected_signature;
  tbge::ecs::Signature signature =
  ...
  expected_signature = tbge::ecs::Signature(0b1);
...
TEST_F(CoordinatorTest, AddComponentsOfDifferentTypes) {
  ...
  signature = test_coordinator->get_entity_manager()->GetSignature(entity);
  expected_signature = tbge::ecs::Signature(0b11);
  EXPECT_EQ(signature, expected_signature)
...
TEST_F(CoordinatorTest, RemoveComponent) {
  ...
  expected_signature = tbge::ecs::Signature(0b10);
  signature = test_coordinator->get_entity_manager()->GetSignature(entity);
  EXPECT_EQ(signature, expected_signature);
}
