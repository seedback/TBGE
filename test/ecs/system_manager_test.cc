...
class SystemManagerTest : public ::testing::Test {
 ...
 protected:
  ...
  tbge::ecs::SystemManager test_system_manager;
};

class DummySystem : public tbge::ecs::System {
 public:
  DummySystem() = default;
};

class DummySystem2 : public tbge::ecs::System {
 public:
...
TEST_F(SystemManagerTest, SetSignature) {
  test_system_manager.RegisterSystem<DummySystem>();
  test_system_manager.SetSignature<DummySystem>(tbge::ecs::Signature(2));

  EXPECT_EQ(test_system_manager.get_signatures().size(), 1);
  EXPECT_EQ(test_system_manager.get_signatures().begin()->second.to_ulong(), 2);

  test_system_manager.SetSignature<DummySystem>(tbge::ecs::Signature(4));
...
TEST_F(SystemManagerTest, EntityDestroyed) {
  ...
  test_system_manager.EntitySignatureChanged(1, tbge::ecs::Signature(2));
  test_system_manager.EntitySignatureChanged(2, tbge::ecs::Signature(2));
  test_system_manager.EntitySignatureChanged(3, tbge::ecs::Signature(2));
  ...
  EXPECT_TRUE(system->has_entity(3)) << "Entity 3 should be in the system";
  ...
  EXPECT_FALSE(system->has_entity(2))
      << "Entity 2 should be removed from the system's entities";
  ...
  EXPECT_EQ(system->get_entities().size(), 2);
}
...
TEST_F(SystemManagerTest, EntityDestroyedWithMultipleSystems) {
  ...
  test_system_manager.SetSignature<DummySystem>(tbge::ecs::Signature(0b01));
  test_system_manager.SetSignature<DummySystem2>(tbge::ecs::Signature(0b10));

  // Add entities to systems by changing their signatures
  test_system_manager.EntitySignatureChanged(1, tbge::ecs::Signature(0b01));
  test_system_manager.EntitySignatureChanged(2, tbge::ecs::Signature(0b01));
  test_system_manager.EntitySignatureChanged(3, tbge::ecs::Signature(0b01));
  test_system_manager.EntitySignatureChanged(4, tbge::ecs::Signature(0b10));
  test_system_manager.EntitySignatureChanged(5, tbge::ecs::Signature(0b10));

  EXPECT_EQ(system1->get_entities().size(), 3)
      << "system1 should have 3 entities";
  EXPECT_EQ(system2->get_entities().size(), 2)
      << "system2 should have 2 entities";
...
TEST_F(SystemManagerTest, EntitySignatureChangedAddAndRemoveEntity) {
  ...
  std::shared_ptr<DummySystem> system =
      test_system_manager.RegisterSystem<DummySystem>();
  test_system_manager.SetSignature<DummySystem>(tbge::ecs::Signature(1));
  ...
  test_system_manager.EntitySignatureChanged(1, tbge::ecs::Signature(1));
  ...
  test_system_manager.EntitySignatureChanged(2, tbge::ecs::Signature(1));
  ...
  test_system_manager.EntitySignatureChanged(2, tbge::ecs::Signature(0));
  ...
  // Change signature of Entity 2 to no longer match system's signature
  test_system_manager.EntitySignatureChanged(1, tbge::ecs::Signature(0));
